...
imagePaths = list(paths.list_images("vediophotoextract"))
maindir = "D:/face-recognition-using-deep-learning-master/dataset/"
...
for (i, imagePath) in enumerate(imagePaths):
    ...
    source = os.path.join(maindir + name)
    try:
    ...
    # load the image, resize it to have a width of 600 pixels (while maintaining the aspect ratio), and then grab the
    # image dimensions
    ...
    # bFromImage	(	InputArray 	image,double 	scalefactor = 1.0,const Size & 	size = Size(),const Scalar & 	mean = Scalar(),bool 	swapRB = false,bool 	crop = false,int 	ddepth = CV_32F )
